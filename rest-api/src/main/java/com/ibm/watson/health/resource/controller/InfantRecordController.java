...
public class InfantRecordController {
	...
	ResponseEntity<?> add(@PathVariable String userId, @RequestBody InfantRecord input) {
    	
		try {
			InfantRecord infantRecord = this.infantRecordService.findById(userId);
			
			HttpHeaders httpHeaders = new HttpHeaders();
			httpHeaders.setLocation(ServletUriComponentsBuilder
					.fromCurrentRequest().path("/{id}")
					.buildAndExpand(infantRecord.getId()).toUri());
